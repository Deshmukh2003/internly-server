...
import com.internly.repository.CompanyRepository;
...
public class CompanyService {
  ...
  public java.util.List<CompanyResponse> list() {
    return companies.findAll().stream().map(this::response).toList();
  }

  @Transactional
  public CompanyResponse create(CompanyRequest r) {
    if (
      companies.findByNameIgnoreCase(r.name().trim()).isPresent()
  ...
  public CompanyResponse update(Long id, CompanyRequest r) {
    ...
    return response(companies.save(c));
  }
  ...
  private CompanyResponse response(Company c) {
