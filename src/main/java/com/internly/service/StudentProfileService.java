...
public class StudentProfileService {
    ...
    private StudentProfile profileFor(String email) { User user = users.findByEmailIgnoreCase(email).orElseThrow(); return profiles.findByUserId(user.getId()).orElseGet(() -> profiles.save(StudentProfile.builder().user(user).skills(new HashSet<>()).build())); }
