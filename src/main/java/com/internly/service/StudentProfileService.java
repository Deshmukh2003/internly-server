...
import com.internly.dto.StudentDtos.*;
import com.internly.entity.*;
import com.internly.integration.storage.FileStorageService;
...
public class StudentProfileService {
  ...
  public ProfileResponse get(String email) {
    return toResponse(profileFor(email));
  }

  @Transactional
  public ProfileResponse save(String email, ProfileRequest request) {
    StudentProfile profile = profileFor(email);
    profile.setFullName(request.fullName().trim());
    profile.setMobile(clean(request.mobile()));
    String domain = clean(request.domain());
    String qualification = clean(request.qualification());
    if (
      domain != null && !StudentProfileOptions.DOMAINS.contains(domain)
    ) throw new IllegalArgumentException(
      "Please select a valid branch / domain"
    );
    if (
      qualification != null &&
    ...
    profile.setCollege(clean(request.college()));
    profile.setGraduationYear(request.graduationYear());
    profile.setInterests(clean(request.interests()));
  ...
  public void removeSkill(String email, Long skillId) {
    ...
    profile
      .getSkills()
      .removeIf(skill -> Objects.equals(skill.getId(), skillId));
    profiles.save(profile);
  }
  ...
  public ProfileResponse uploadResume(String email, MultipartFile file) {
    ...
    StudentProfile profile = profileFor(email);
    try {
      if (profile.getResumeStorageKey() != null) storage.delete(
        profile.getResumeStorageKey()
      );
      profile.setResumeStorageKey(storage.store(file));
      profile.setResumeFileName(file.getOriginalFilename());
      profile.setResumeUploadedAt(Instant.now());
      return toResponse(profiles.save(profile));
    } catch (IOException ex) {
      throw new IllegalArgumentException("Unable to store resume");
    }
  }
  ...
  public void deleteResume(String email) {
    StudentProfile profile = profileFor(email);
    try {
      storage.delete(profile.getResumeStorageKey());
    } catch (IOException ex) {
      throw new IllegalArgumentException("Unable to delete resume");
    }
    profile.setResumeStorageKey(null);
    profile.setResumeFileName(null);
    profile.setResumeUploadedAt(null);
    profiles.save(profile);
  }

  private StudentProfile profileFor(String email) {
    User user = users.findByEmailIgnoreCase(email).orElseThrow();
    return profiles
      .findByUserId(user.getId())
      .orElseGet(() ->
        profiles.save(
          StudentProfile.builder().user(user).skills(new HashSet<>()).build()
        )
      );
  }

  private ProfileResponse toResponse(StudentProfile p) {
    List<SkillResponse> skillResponses = p
      .getSkills()
      .stream()
      .sorted(
    ...
    return new ProfileResponse(
      ...
      p.getInterests(),
      p.getResumeFileName(),
      p.getResumeUploadedAt(),
      skillResponses
    );
  }

  private void validateResume(MultipartFile file) {
    if (file == null || file.isEmpty()) throw new IllegalArgumentException(
      "Please select a resume"
    );
    if (file.getSize() > 5 * 1024 * 1024) throw new IllegalArgumentException(
